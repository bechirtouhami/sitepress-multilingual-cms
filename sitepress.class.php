...
class SitePress{
    ...
    function the_posts($posts){        
        ...
        //exceptions
        if( (($wp_query == $custom_wp_query) && ($wp_query->is_singular || $posts[0]->post_type == 'page')) 
        ...
        // cat
        if(isset($custom_wp_query->query_vars['cat']) && !empty($custom_wp_query->query_vars['cat'])){
            $custom_wp_query->query_vars['cat'] = icl_object_id($custom_wp_query->query_vars['cat'], 'category', true);
        }                
        // category_name
        if(isset($custom_wp_query->query_vars['category_name']) && !empty($custom_wp_query->query_vars['category_name'])){
            $cat_id = $wpdb->get_var("
                SELECT x.term_taxonomy_id FROM $wpdb->term_taxonomy x JOIN $wpdb->terms t ON t.term_id = x.term_id
                WHERE x.taxonomy='category' AND t.slug='".$wpdb->escape($custom_wp_query->query_vars['category_name'])."'
            ");            
            $translated_cat_id = icl_object_id($cat_id, 'category', true, $this->get_default_language());
            $translated_cat_id = $wpdb->get_var("SELECT term_id FROM $wpdb->term_taxonomy WHERE term_taxonomy_id=" . $translated_cat_id);
            $custom_wp_query->query_vars['cat'] = $translated_cat_id;
            unset($custom_wp_query->query_vars['category_name']);
        }
        // category__and
        if(isset($custom_wp_query->query_vars['category__and']) && !empty($custom_wp_query->query_vars['category__and'])){
            $translated_cat_ids = array();
            foreach($custom_wp_query->query_vars['category__and'] as $c){
                $translated_cat_ids[] = icl_object_id($c, 'category', true, $this->get_default_language());
            }
            $custom_wp_query->query_vars['category__and'] = $translated_cat_ids;
        }
        // category__in
        if(isset($custom_wp_query->query_vars['category__in']) && !empty($custom_wp_query->query_vars['category__in'])){
            $translated_cat_ids = array();
            foreach($custom_wp_query->query_vars['category__in'] as $c){
                $translated_cat_ids[] = icl_object_id($c, 'category', true, $this->get_default_language());
            }
            $custom_wp_query->query_vars['category__in'] = $translated_cat_ids;
        }
        // category__not_in
        if(isset($custom_wp_query->query_vars['category__not_in']) && !empty($custom_wp_query->query_vars['category__not_in'])){
            $translated_cat_ids = array();
            foreach($custom_wp_query->query_vars['category__not_in'] as $c){
                $translated_cat_ids[] = icl_object_id($c, 'category', true, $this->get_default_language());
            }
            $custom_wp_query->query_vars['category__not_in'] = $translated_cat_ids;
        }
        
        // TAGS
        // tag
        
        if(isset($custom_wp_query->query_vars['tag']) && !empty($custom_wp_query->query_vars['tag'])){
            if(false !== strpos($custom_wp_query->query_vars['tag'],'+')){
                $tag_glue = '+';
                $exp = explode('+', $custom_wp_query->query_vars['tag']);
            }else{
                $tag_glue = ',';
                $exp = explode(',', $custom_wp_query->query_vars['tag']);                    
            } 
            ...
            if(!empty($tag_array)){
                ...
                foreach($tag_array as $c){                    
                    if(intval($c) < 0){
                     ...
                     $_tid = intval(icl_object_id(abs($c), 'post_tag', true, $this->get_default_language()));
            ...
            $custom_wp_query->query_vars['tag'] = join($tag_glue, $slugs);    
        }
        
        // tag_id   
        if(isset($custom_wp_query->query_vars['tag_id']) && !empty($custom_wp_query->query_vars['tag_id'])){
            $tag_array = array_map('trim', explode(',', $custom_wp_query->query_vars['tag_id']));
            if(!empty($tag_array)){
                $translated_ids = array();
                foreach($tag_array as $c){                    
                    if(intval($c) < 0){
                ...
                $custom_wp_query->query_vars['tag_id'] = join(',', $translated_ids);                
            }            
        }         
        
        // tag__and   
        if(isset($custom_wp_query->query_vars['tag__and']) && !empty($custom_wp_query->query_vars['tag__and'])){
            $translated_ids = array();
            foreach($custom_wp_query->query_vars['tag__and'] as $t){
                $translated_ids[] = icl_object_id($t, 'post_tag', true, $this->get_default_language());
            }
            $custom_wp_query->query_vars['tag__and'] = $translated_ids;
        }         
        
        // tag__in   
        if(isset($custom_wp_query->query_vars['tag__in']) && !empty($custom_wp_query->query_vars['tag__in'])){
            $translated_ids = array();
            foreach($custom_wp_query->query_vars['tag__in'] as $t){
                $translated_ids[] = icl_object_id($t, 'post_tag', true, $this->get_default_language());
            }
            $custom_wp_query->query_vars['tag__in'] = $translated_ids;            
        }         
        
        // tag__not_in   
        if(isset($custom_wp_query->query_vars['tag__not_in']) && !empty($custom_wp_query->query_vars['tag__not_in'])){
            $translated_ids = array();
            foreach($custom_wp_query->query_vars['tag__not_in'] as $t){
                $translated_ids[] = icl_object_id($t, 'post_tag', true, $this->get_default_language());
            }
            $custom_wp_query->query_vars['tag__not_in'] = $translated_ids;            
        }         
        
        // tag_slug__in
        if(isset($custom_wp_query->query_vars['tag_slug__in']) && !empty($custom_wp_query->query_vars['tag_slug__in'])){
            $tag_array = array();
            foreach($custom_wp_query->query_vars['tag_slug__in'] as $t){
                $tag_array[] = $wpdb->get_var($wpdb->prepare( "SELECT x.term_taxonomy_id FROM $wpdb->terms t 
                    JOIN $wpdb->term_taxonomy x ON t.term_id=x.term_id WHERE x.taxonomy='post_tag' AND t.slug='%s'", $wpdb->escape($t)));    
            }
            $translated_ids = array();
            foreach($tag_array as $c){                    
                 $_tid = icl_object_id($c, 'post_tag', true, $this->get_default_language());
                 $translated_ids[] = $wpdb->get_var($wpdb->prepare("SELECT term_id FROM $wpdb->term_taxonomy WHERE term_taxonomy_id=%d", $_tid));
            }
            $custom_wp_query->query_vars['tag_slug__in'] = $translated_ids;
        }         
        
        // tag_slug__and
        if(isset($custom_wp_query->query_vars['tag_slug__and']) && !empty($custom_wp_query->query_vars['tag_slug__and'])){
            $tag_array = array();
            foreach($custom_wp_query->query_vars['tag_slug__and'] as $t){
                $tag_array[] = $wpdb->get_var($wpdb->prepare( "SELECT x.term_taxonomy_id FROM $wpdb->terms t 
                    JOIN $wpdb->term_taxonomy x ON t.term_id=x.term_id WHERE x.taxonomy='post_tag' AND t.slug='%s'", $wpdb->escape($t)));    
            }
            $translated_ids = array();
            ...
            $custom_wp_query->query_vars['tag_slug__and'] = $translated_ids;
        }         
        
        // POST & PAGES
        // page_id
        if(isset($custom_wp_query->query_vars['page_id']) && !empty($custom_wp_query->query_vars['page_id'])){
            $custom_wp_query->query_vars['page_id'] = icl_object_id($custom_wp_query->query_vars['page_id'], 'page', true, $this->get_default_language());
        }
        // p
        if(isset($custom_wp_query->query_vars['p']) && !empty($custom_wp_query->query_vars['p'])){
            $custom_wp_query->query_vars['p'] = icl_object_id($custom_wp_query->query_vars['p'], 'post', true, $this->get_default_language());
        }
        // name
        if(isset($custom_wp_query->query_vars['name']) && !empty($custom_wp_query->query_vars['name'])){
            $pid = $wpdb->get_var("SELECT ID FROM $wpdb->posts WHERE post_name='".$wpdb->escape($custom_wp_query->query_vars['name'])."'");
            $custom_wp_query->query_vars['p'] = icl_object_id($pid, 'post', true, $this->get_default_language());
            unset($custom_wp_query->query_vars['name']);
        }
        // pagename
        if(isset($custom_wp_query->query_vars['pagename']) && !empty($custom_wp_query->query_vars['pagename'])){
            $pid = $wpdb->get_var("SELECT ID FROM $wpdb->posts WHERE post_name='".$wpdb->escape($custom_wp_query->query_vars['pagename'])."'");
            $custom_wp_query->query_vars['page_id'] = icl_object_id($pid, 'page', true, $this->get_default_language());
            unset($custom_wp_query->query_vars['pagename']);
        }
        // post__in
        if(isset($custom_wp_query->query_vars['post__in']) && !empty($custom_wp_query->query_vars['post__in'])){
            $pid = array();
            foreach($custom_wp_query->query_vars['post__in'] as $p){
                $pid[] = icl_object_id($p, 'post', true, $this->get_default_language());                    
            }
            $custom_wp_query->query_vars['post__in'] = $pid;
        }
        // post__not_in
        if(isset($custom_wp_query->query_vars['post__not_in']) && !empty($custom_wp_query->query_vars['post__not_in'])){
            $pid = array();
            foreach($custom_wp_query->query_vars['post__not_in'] as $p){
                $pid[] = icl_object_id($p, 'post', true, $this->get_default_language());                    
            }
            $custom_wp_query->query_vars['post__not_in'] = $pid;
        }
        // post_parent
    ...
    /*    
    function the_posts_ORIGINAL($posts){        
        global $wpdb, $wp_query;
