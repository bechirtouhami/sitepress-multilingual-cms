    <div class="icl_advanced_switch">        
        <?php if(!$sitepress_settings['advanced']): ?>
        <span><?php printf(__('Using WordPress as CMS? Try WPML&#39;s <span title="%s">advanced features</span>.','sitepress'),__('Theme localization, translation synchronization, comments translation, CMS navigation, Sticky links','sitepress')) ?> <a class="button icl_golden_button" href="admin.php?page=<?php echo $_GET['page'] ?>&amp;icl_action=advanced&amp;nonce=<?php echo wp_create_nonce('icl_enable_advanced_mode') ?>" title="<?php _e("Switch to advanced setup mode", 'sitepress') ?>"><?php _e('Go Advanced', 'sitepress')?></a></span>
        <?php else: ?>
        <?php 
