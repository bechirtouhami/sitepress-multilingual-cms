...
<?php
		...
		foreach ($this->tickets as $id => $v) {
			...
			if (!$this->initial && $v['messages'] != $this->fetched_tickets[$id]['messages']) {
				...
				$updated_tickets .= '<tr'.$add.'><td>' . $this->thickbox($url . $v['id'], $add3, $add4) . $v['subject'] . '</a></td><td>' . date(get_option('date_format'), $v['create_time']) . '</td><td>' . $v['messages'] . '&nbsp;' . $add2 . '</td></tr>';
			} else {
...
?>             
                            ...
                            <!--admin.php?page=<?php echo ICL_PLUGIN_FOLDER  ?>/menu/content-translation.php#icl_create_account_form-->
                                ...
                                <p style="line-height:1.5"><?php _e('To get premium support, you will need to create an account at ICanLocalize. WPML will use this account to create support tickets and connect you with the development team.', 'sitepress'); ?></p>
