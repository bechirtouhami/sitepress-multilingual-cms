    ...
    <ul>
        <li><a href="#" class="lang_sel_sel"><?php echo $wpdb->get_var("SELECT name FROM {$wpdb->prefix}icl_languages_translations WHERE language_code='{$w_this_lang['code']}' AND display_language_code='{$w_this_lang['code']}'"); ?><?php if(!isset($ie_ver) || $ie_ver > 6): ?></a><?php endif; ?>
            ...
            <ul>
                ...
                <li><a href="<?php echo $lang['translated_url']?>"><?php echo $wpdb->get_var("SELECT name FROM {$wpdb->prefix}icl_languages_translations WHERE language_code='{$lang['code']}' AND display_language_code='{$lang['code']}'");?> (<?php echo $wpdb->get_var("SELECT name FROM {$wpdb->prefix}icl_languages_translations WHERE language_code='{$lang['code']}' AND display_language_code='{$w_this_lang['code']}'");?>)</a></li>
                <?php endforeach; ?>
            </ul>
