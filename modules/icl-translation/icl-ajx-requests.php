            ...
            if ($target_languages != null){
                ...
                foreach($target_languages as $l){
                    ...
                    $lang_loc = $wpdb->get_var("
                        ...
                        WHERE lt.display_language_code='" . $sitepress->get_default_language(). "' AND l.name='{$lang}'");
                    if(!$lang_loc){
                        $lang_loc = $lang;
                    }
                    ...
                    if($l['translator']['attr']['id']){
                        $language_status[$lang]['translator'] = '<a class="icl_thickbox_insert" href="' . ICL_PLUGIN_URL . "/modules/icl-translation/icl-reminder-popup.php?target=" .ICL_API_ENDPOINT .'/websites/'.$iclq->setting('site_id').'/cms_requests/'.$rid.'/chat?lang='.str_replace(' ','%20',$lang_server).'">'.$l['translator']['attr']['nickname'].'</a>';
                    } else {
                        $language_status[$lang]['translator'] = __('None assigned', 'sitepress');
                    }
                    $language_status[$lang]['project'] = '<a class="icl_thickbox_insert" href="' . ICL_PLUGIN_URL . "/modules/icl-translation/icl-reminder-popup.php?target=" .ICL_API_ENDPOINT.'/websites/'.$iclq->setting('site_id').'/cms_requests/'.$rid.'">'.__('Project page on ICanLocalize.com', 'sitepress').'</a>';
